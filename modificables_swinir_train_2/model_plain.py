...
class ModelPlain(ModelBase):
    ...
    def init_train(self):
        ...
        #"layers.4",             # últimos RSTB
        #"layers.5",

        print("\n🔥 Layers que se entrenarán:\n")
        ...
        for name, param in self.netG.named_parameters():
            if any(layer in name for layer in trainable_layers):
                ...
                activated += param.numel()
                print(name)

        # --------------------------------
        # conteo de parámetros
        # --------------------------------
        total_params = sum(p.numel() for p in self.netG.parameters())
        trainable_params = sum(p.numel() for p in self.netG.parameters() if p.requires_grad)

        print("\n📊 Parámetros entrenables:", trainable_params)
        print("📊 Parámetros totales:", total_params)
        print("📊 % entrenando:", 100 * trainable_params / total_params, "%\n")

        # --------------------------------
        # seguridad: verificar que sí haya capas activas
        # --------------------------------
        if trainable_params == 0:
            raise ValueError("⚠️ No hay parámetros entrenables. Revisa los nombres de capas.")
